...
static void partition_read_block ( wiidisc_t *d, u32 blockno, u8 *block )
{
    ...
    offset = d->partition_data_offset + ((WII_SECTOR_SIZE>>2) * blockno );
    partition_raw_read(d,offset, raw, WII_SECTOR_SIZE);
...
static int test_partition_skip ( u32 partition_type, partition_selector_t part_sel )
{
	...
	case REMOVE_GAME_PARTITION:	return partition_type == GAME_PARTITION_TYPE;
...
static void do_disc ( wiidisc_t * d )
{
	...
	wbfs_fatal("%d partitions found but can only handle %s partitions.\n",
		n_partitions, MAX_PARTITIONS );

    // get partition info
    disc_read( d, _be32(b + 4), b, BUFSIZE );
