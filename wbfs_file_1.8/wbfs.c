...
int get_first_disc_id(wbfs_t *p, char *discid)
{
    ...
    u8 b[0x100];
    if(wbfs_get_disc_info(p,0,b,0x100,&size)) return -1;
    memcpy(discid, b, 6);
    discid[6] = 0;
    return 0;
}
...
void usage(char **argv)
{
    ...
    printf("    -s SIZE  :  Set split size ["FMT_lld"] ", OPT_split_size);
    printf("(%d sectors)\n", (u32)(OPT_split_size/512));
...
int main(int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "hag124s:p:d:ftw")) != -1) {
        switch (opt) {
            /*case 'p':
              partition = optarg;
              break;
